...
IWidget *CBaseContainer::ChildByPoint(int x, int y)
{
    for (int i = ChildCount() - 1; i >= 0; i--)
    {
        ...
        if (x >= co.first && x <= co.first + cs.first && y >= co.second &&
            y <= co.second + cs.second)
        {
            return child;
        }
    }
...
void CBaseContainer::OnMousePress()
{
    ...
    PressOn(ChildByPoint(g_pGUI->m_iMouseX - abs.first,
                         g_pGUI->m_iMouseY - abs.second));
}
...
{
    auto abs     = AbsolutePosition();
    auto hovered = ChildByPoint(g_pGUI->m_iMouseX - abs.first,
                                g_pGUI->m_iMouseY - abs.second);
